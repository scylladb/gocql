...
// metrics pointer for reuse and allocates a fresh routingInfo (to avoid
// racing with paging copies that share the routingInfo pointer).
func TestQueryResetPreservesAllocations(t *testing.T) {
	...
	// routingInfo must be a NEW allocation (not reused) to avoid races with paging copies.
	if q.routingInfo == origRI {
		t.Fatal("reset() reused routingInfo pointer; expected a fresh allocation")
	}
	// metrics pointer must be preserved (same address).
	if q.metrics != origM {
		t.Fatal("reset() allocated a new metrics instead of reusing the existing one")
	}

	// routingInfo fields must be zeroed.
...
// gracefully (e.g. for manually constructed Query objects not obtained from the pool).
func TestQueryResetNilMetrics(t *testing.T) {
	...
	if q.routingInfo == nil {
		t.Fatal("reset() did not set routingInfo")
	}
	if q.metrics != nil {
...
func TestQueryPoolRoundTrip(t *testing.T) {
	...
	if q2 == q {
		// routingInfo is freshly allocated on each reset (not reused), so just check it exists.
		if q2.routingInfo == nil {
			t.Fatal("routingInfo is nil after pool round-trip")
		}
		if q2.metrics != origM {
