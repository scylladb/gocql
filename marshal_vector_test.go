...
// wireVectorType parses a custom column type the way readTypeInfo does, so the
// TypeInfo under test is one a server can actually install.
func wireVectorType(custom string) TypeInfo {
	f := &framer{proto: protoVersion4}
	return f.readVectorTypeInfo(NewCustomType(protoVersion4, TypeCustom, custom), apacheCassandraTypePrefix+"VectorType")
}
