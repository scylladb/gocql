...
type SegmentSplitter struct {
	...
	// failed is the error that ended this splitter, if one did. Like the frame
	// splitter it wraps, it is one-shot: see Feed.
	failed error
...
//     endless chain that never progresses.
...
//
// A failure here means the segment stream cannot be trusted from this point: the
// buffer is positioned at a segment that did not decode, or whose payload was already
// handed on before the check that rejected it. Resuming would re-feed those bytes. The
// wrapped frame splitter is one-shot for the same reason.
func (s *SegmentSplitter) fail(err error) error {
	if s.failed == nil {
		s.failed = err
	}
	return s.failed
