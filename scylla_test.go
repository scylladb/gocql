...
func TestScyllaRateLimitingExtParsing(t *testing.T) {
	...
	t.Run("init framer without cql extensions", func(t *testing.T) {
		...
		conn := mockConn(0)
		f := newFramerWithExts(conn.compressor, conn.version, conn.cqlProtoExts, nopLogger{})
	...
	t.Run("init framer with cql extensions", func(t *testing.T) {
		...
		framerWithRateLimitExt := newFramerWithExts(conn.compressor, conn.version, conn.cqlProtoExts, nopLogger{})
