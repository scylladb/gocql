...
for tier in "${TIERS[@]}"; do
	...
	for rep in $(seq 1 "$REPEATS"); do
		...
		# Rotate the order per repeat so any repeat that lands first isn't
		# always the same config, and record what ran.
		n=${#CONFIGS[@]}
		shift=$(((rep - 1) % n))
		rep_configs=("${CONFIGS[@]:shift}" "${CONFIGS[@]:0:shift}")
		{
			printf 'rep %s order:' "$rep"
			for c in "${rep_configs[@]}"; do printf ' %s' "${c%%:*}"; done
			printf '\n'
		} >> "$OUTDIR/experiment.txt"
		for cfg in "${rep_configs[@]}"; do
			label="${cfg%%:*}"
			...
			before=$(out_segs)
			# ponytail: E5 TLS runs still lack TLS-record-rate and server-CPU
			# instrumentation -- no reachable hook exists (no perf/nodetool/
			# prometheus counter wired here). Add when that plumbing exists;
			# see CodeRabbit finding on this PR.
			/usr/bin/time -f "%U %S %e" -o "$out.cpu" \
				...
				"${extra[@]}" \
				> "$out.txt" 2>&1 || { echo -n "FAILED "; echo "FAILED" >> "$out.txt"; }
			after=$(out_segs)
