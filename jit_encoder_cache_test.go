...
func TestGetOrCompileParamEncoderCachedHitsAndMisses(t *testing.T) {
	...
	enc1 := getOrCompileParamEncoderCached(stmt, columns, valuesA)
	if enc1 == nil {
	...
	enc2 := getOrCompileParamEncoderCached(stmt, columns, valuesA2)
	if enc1 != enc2 {
	...
	enc3 := getOrCompileParamEncoderCached(stmt, columns, valuesB)
	if enc3 == enc1 {
	...
	enc4 := getOrCompileParamEncoderCached(stmt, columns, valuesA)
	if enc4 != enc1 {
...
func TestGetOrCompileParamEncoderCachedNilStmt(t *testing.T) {
	...
	enc := getOrCompileParamEncoderCached(nil, columns, values)
...
func BenchmarkGetOrCompileParamEncoder_ColdVsCachedHit(b *testing.B) {
	...
	b.Run("Cached_PerStatement_WarmHit", func(b *testing.B) {
		stmt := &preparedStatment{}
		getOrCompileParamEncoderCached(stmt, columns, values) // warm the cache
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_ = getOrCompileParamEncoderCached(stmt, columns, values)
		}
	})
}
