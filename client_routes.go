...
func (r *simplePortMuxResolver) Resolve(endpoint ResolvedClientRoute) (allKnown []net.IP, current net.IP, err error) {
	r.mu.RLock()
...
func (p *ClientRoutesHandler) TranslateWithHost(hostID string, addr net.IP, port int) (net.IP, int, error) {
	if hostID == "" {
	...
	if rec == nil {
		return addr, port, fmt.Errorf("no address found for host %s", hostID)
	}

	if rec.currentIP != nil {
		return rec.currentIP, int(rec.GetCQLPort()), nil
	}

	all, currentIP, err := p.cfg.DNSResolver.Resolve(*rec)
	if err != nil {
		return addr, port, fmt.Errorf("failed to resolve DNS resolver for host %s: %v", hostID, err)
	}
	rec.allKnownIPs = all
...
func (p *ClientRoutesHandler) getResolveHostPortMapping() *ResolvedClientRouteList {
	endpoints := p.resolvedEndpoints.Load()
	if endpoints == nil {
		return &ResolvedClientRouteList{}
	}
	return endpoints
}
