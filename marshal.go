...
func Marshal(info TypeInfo, value any) ([]byte, error) {
	...
	case TypeList, TypeSet:
		...
		if !ok {
			return nil, marshalErrorf("can not marshal %s into %T", info, value)
		}
...
// These read from raw bytes using encoding/binary.BigEndian directly,
// reusing the destination slice's backing array when cap >= dim.
...
func unmarshalListInt(data []byte, dst *[]int) error {
	...
	vec := *dst
	if n == 0 {
		// n==0: emit a non-nil empty []int (matches other leaf decoders and
		// TestMarshalSetListV3/zero_elems, which expects a non-nil empty slice).
		vec = make([]int, 0)
	} else if cap(vec) >= n {
		vec = vec[:n]
	} else {
