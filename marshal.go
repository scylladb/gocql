...
func marshalVector(info VectorType, value any) ([]byte, error) {
	...
	//
	// Zero-dimension vectors fall through to the generic path on purpose: it
	// returns nil (CQL null) for an empty buffer, whereas the float fast paths
	// return a non-nil zero-length slice. Mirroring unmarshalVector's
	// `info.Dimensions > 0` guard keeps the wire value consistent.
...
func unmarshalVector(info VectorType, data []byte, value any) error {
	...
	// Only taken for the common case (positive dimensions with data present); nil/empty and
	// zero-dimension edge cases fall through to the generic path, which carries the canonical
	// semantics (non-nil empty slice, 0-dimension error messages).
