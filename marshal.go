...
func marshalVector(info VectorType, value interface{}) ([]byte, error) {
	...
	case reflect.Slice, reflect.Array:
		...
		// Pre-allocate buffer for fixed-size element types to avoid repeated growth.
		buf := &bytes.Buffer{}
		if !isLengthType {
			if elemSize := vectorFixedElemSize(info.SubType); elemSize > 0 {
				// Use 64-bit arithmetic to avoid int overflow on large Dimensions.
...
// vectorFixedElemSize returns the fixed serialized byte size for a CQL type used as a vector element.
// It only covers types that isVectorVariableLengthType classifies as fixed-length (returns false for).
// Returns 0 for variable-length types or types with unknown fixed sizes.
