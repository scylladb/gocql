...
func TestMarshalListFastPath_Int_OutOfRange(t *testing.T) {
	if math.MaxInt <= math.MaxInt32 {
		t.Skip("int is 32-bit on this platform; no int value is out of int32 range")
	}
	info := listTypeInfo(TypeInt)
	// Compute at runtime: int(math.MaxInt32)+1 is a compile-time constant
	// that doesn't fit in a 32-bit int, which would fail to build there.
	outOfRange := int64(math.MaxInt32) + 1
	if _, err := marshalList(info, []int{int(outOfRange)}); err == nil {
