...
func TestSegmentSplitterFeedReusesItsBuffers(t *testing.T) {
	for _, cc := range compressors() {
		t.Run(cc.name, func(t *testing.T) {
			...
			// Bytes rather than allocation count, as internal/segment's own reuse
			// test does. The budget is per segment: a bytes.Reader lands in the
			// 48-byte size class and the spelling this replaced made two of them per
			// segment, so 64 bytes each catches the pair with room to spare for the
			// couple of small fixed allocations the codec still makes. Measured at 16
			// bytes per segment, or 32 under -race.
			const perSegmentBudget = 64
			if allocated := after.TotalAlloc - before.TotalAlloc; allocated > feeds*perSegmentBudget {
				t.Errorf("decoding %d segments allocated %d bytes, want at most %d — a buffer is not being reused",
					feeds, allocated, feeds*perSegmentBudget)
			}
		})
	}
}
