...
func (c *controlConn) heartBeat() {
	...
	for {
		...
		// Skip heartbeat if the underlying connection had inbound activity since
		// the last check (only when HeartbeatSkipOnActivity is enabled).
		if c.session.cfg.HeartbeatSkipOnActivity {
