...
// and optionally the decoded names.
var unsetMarker = []byte("<unset>")

func decodeQueryParamsValues(t *testing.T, body []byte, flags uint32, named bool) (names []string, values [][]byte) {
	t.Helper()
	...
	for i := 0; i < n; i++ {
		if named {
			nl := readShort()
			names[i] = string(body[pos : pos+nl])
			pos += nl
		}
		size := readInt()
		switch {
...
func buildQueryParamsFrame(t *testing.T, proto byte, params queryParams) *framer {
	t.Helper()
...
func TestWriteQueryParamsValuesRoundTrip(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			f := buildQueryParamsFrame(t, protoVersion4, params)
			...
			// When values is nil/empty writeQueryParams emits no value-count
			// short, so skip the full decode+compare for those cases.
			if len(tc.values) > 0 {
				_, got := decodeQueryParamsValues(t, f.buf[off:], 0, false)
...
func TestWriteQueryParamsNamedValuesRoundTrip(t *testing.T) {
	...
	gotNames, got := decodeQueryParamsValues(t, f.buf[off:], 0, true)
	if len(got) != len(values) {
		t.Fatalf("got %d values, want %d", len(got), len(values))
	}
	for i, qv := range values {
		if gotNames[i] != qv.name {
			t.Fatalf("value %d: got name %q, want %q", i, gotNames[i], qv.name)
		}
		if !bytes.Equal(got[i], qv.value) {
