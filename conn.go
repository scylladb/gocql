...
func (c *Conn) recvSegment(ctx context.Context) error {
	...
	// Mirror the timeout-disarm/re-arm logic from processFrame: the serve()
	// loop must block indefinitely on an idle connection waiting for the first
	// bytes of a segment, so we clear the read deadline before the initial
	// segment read and restore it afterwards so that all subsequent body reads
	// (payload bytes, CRC, partial-frame continuations) remain bounded by
	// ReadTimeout.
	...
	// Restore per-request read timeout before any further body reads.
	if savedTimeout > 0 {
		c.r.SetTimeout(savedTimeout)
	}

	if err != nil {
		// Normalise a network read-timeout on the segment header into
		// ErrReadHeaderTimeout so that serve() treats it as non-fatal (idle
		// connection with no in-flight requests) instead of closing the
		// connection.
		var netErr net.Error
		if errors.As(err, &netErr) && netErr.Timeout() {
			return fmt.Errorf("%w: %w", ErrReadHeaderTimeout, err)
		}
		return err
	}
...
func (c *Conn) executeQuery(ctx context.Context, qry *Query) (iter *Iter) {
	...
	if !qry.skipPrepare && qry.shouldPrepare() {
		...
		// When SCYLLA_USE_METADATA_ID extension is negotiated, the server sends METADATA_CHANGED
		// flag when the result metadata changes, so it is safe to skip metadata even if the
		// session-level DisableSkipMetadata flag is set.
		disableSkipMeta := qry.disableSkipMetadata || (!c.scyllaUseMetadataId && c.session.cfg.DisableSkipMetadata)
		params.skipMeta = !disableSkipMeta && len(info.response.columns) != 0
