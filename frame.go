...
func (e *writeExecuteFrame) buildFrame(fr *framer, streamID int) error {
	return fr.writeExecuteFrame(streamID, e.preparedID, &e.params, &e.customPayload, e.noCompress)
}

func (f *framer) writeExecuteFrame(streamID int, preparedID []byte, params *queryParams, customPayload *map[string][]byte, noCompress bool) error {
	if len(*customPayload) > 0 {
		f.payload()
	}
	flags := f.flags
