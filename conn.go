...
func (w *writeCoalescer) flush(resultChans []chan<- writeResult, buffers net.Buffers) {
	...
	// append() only overwrites up to len(buffers); clear any tail left over
	// from a larger prior batch so its byte slices can be GC'd.
	clear(w.scratchBufs[len(w.scratchBufs):cap(w.scratchBufs)])
