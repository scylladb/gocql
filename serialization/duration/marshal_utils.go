...
const (
	...
	nanoDay    = 24 * 60 * 60 * 1000 * 1000 * 1000
	nanoDayNeg = -nanoDay
)
...
func encDur(v time.Duration) []byte {
	if v < nanoDay && v > nanoDayNeg {
		return encNanos(encIntZigZagDur(v))
	}
	n := v % nanoDay
	return encDaysNanos(encIntZigZag32(int32((v-n)/nanoDay)), encIntZigZagDur(n))
}
...
func encInt64(v int64) []byte {
	...
	n := v % nanoDay
	return encDaysNanos(encIntZigZag32(int32((v-n)/nanoDay)), encIntZigZag64(n))
}
