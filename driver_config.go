...
type connectionReport struct {
	...
	// There is no NodePreference field: the schema's optional
	// connection.node-preference describes which nodes the driver opens
	// connections *to*, a different claim from
	// query.load-balancing.node-preference (which nodes get picked for
	// routing). gocql's connection pool has no DC/rack-scoped membership of
	// its own -- Session.init opens a pool for every host that passes
	// cfg.HostFilter, regardless of what the host selection policy's DC/rack
	// preference is. Reporting the load-balancing preference here too would
	// be a factually wrong claim about the pool, so it's left omitted.
}
