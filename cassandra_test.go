...
func TestPrepareExecuteScyllaMetadataChangedFlag(t *testing.T) {
	...
	if err := createTable(session, "DROP TABLE IF EXISTS gocql_test.scylla_metadata_changed"); err != nil {
		t.Fatal(err)
	}
	if err := createTable(session, "CREATE TABLE gocql_test.scylla_metadata_changed(id int, PRIMARY KEY (id))"); err != nil {
		t.Fatal(err)
	}
