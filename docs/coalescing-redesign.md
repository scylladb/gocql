...
F8. **MSS, not "1400".** Linux MSS = MTU − 40 − 12(ts) = **1448** for MTU 1500 (IPv6: 1428). Cloud: AWS 9001 inside a VPC incl. cross-AZ (MSS 8949), 1500 to internet/inter-region; GCP default MTU **1460** (MSS 1408), configurable; Azure 1500. The negotiated value is available per socket via `getsockopt(TCP_MAXSEG)` through `(*net.TCPConn).SyscallConn()`. Rationale for MSS as the threshold: once ≥1 MSS is buffered the kernel emits a full segment anyway; waiting longer only adds segments. A larger k×MSS threshold saves *syscalls* (TSO/GSO handle multi-MSS cheaply) — a CPU objective, not a packet objective. **Closed:** `WrapTLS` (dial.go) now probes MSS via `getsockopt(TCP_MAXSEG)` at dial time and subtracts a fixed TLS overhead estimate, falling back to the 1448 default when probing isn't possible.
