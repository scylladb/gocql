...
func (c *Conn) init(ctx context.Context, dialedHost *DialedHost) error {
	...
	if c.session.cfg.WriteCoalesceWaitTime > 0 && !c.cfg.disableCoalesce && !dialedHost.DisableCoalesce {
		...
		c.w = newWriteCoalescer(dialedHost.Conn, c.cfg.ConnectTimeout, window, coalesceThresholdFor(dialedHost.Conn), ctx.Done())
	}
...
// maxCoalesceBatchLen: request-count cap past which a flush's buffers and
// resultChans slices are freed instead of retained, so one huge batch (e.g.
// tens of thousands of requests) doesn't pin its peak capacity forever.
const maxCoalesceBatchLen = 256
...
// coalesceFlushThresholdTLS caps the threshold below the ~1186-byte record
// size crypto/tls starts at under TLS 1.3 adaptive sizing; a higher
// threshold would make a flush's Write split into two TLS records.
const coalesceFlushThresholdTLS = 1100

// coalesceThresholdFor picks the flush threshold for conn, capping it for
// TLS connections. See coalesceFlushThreshold and coalesceFlushThresholdTLS.
func coalesceThresholdFor(conn net.Conn) int {
	if _, ok := conn.(*tls.Conn); ok {
		return coalesceFlushThresholdTLS
	}
	return coalesceFlushThreshold
}
...
func (w *writeCoalescer) writeFlusherImpl(timerC <-chan time.Time, resetTimer, stopTimer func()) {
	...
	doFlush := func() {
		...
		clear(buffers)
		clear(resultChans)
		if cap(buffers) > maxCoalesceBatchLen {
			// An outsized batch (e.g. tens of thousands of requests) would
			// otherwise pin its capacity for the connection's lifetime.
			buffers = nil
			resultChans = nil
		} else {
			buffers = buffers[:0]
...
func (w *writeCoalescer) flush(resultChans []chan<- writeResult, buffers net.Buffers) {
	...
	if len(buffers) == 1 {
		...
		// the life of the connection. Compare against this connection's own
		// threshold, not the global constant, so large-MSS conns keep their
		// scratch buffer instead of reallocating every flush.
		if cap(w.scratchBuf) > 4*w.flushThreshold {
			w.scratchBuf = nil
		}
	}
	n := int64(wn)
