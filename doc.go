...
// cannot tell whether a given payload is actually compressible. Prepared
// statements with vector/embedding columns — dense, high-entropy floating point
// values that compress poorly — automatically skip compression, regardless of
// CompressionPolicy. For other statements known to carry incompressible data,
// use Query.NoCompression(true) (or Batch.NoCompression(true)) to unconditionally
// skip compression for that statement. This only affects EXECUTE and BATCH
// frames (prepared, DML statements); it has no effect on non-DML statements,
// which are never sent as EXECUTE frames.
