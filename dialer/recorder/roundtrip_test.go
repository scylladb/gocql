...
func TestProtocolV5QueryReplaysWithAFreshTimestamp(t *testing.T) {
	...
	recorded := append(append([]exchange{}, handshake...), queries(111)...)
	fname := filepath.Join(t.TempDir(), "conn")

	rec, err := NewConnectionRecorder(fname, &stubConn{readData: wire(t, recorded, nil, true)}, nil)
	if err != nil {
		t.Fatalf("NewConnectionRecorder: %v", err)
	}
	for i, ex := range recorded {
		if _, err := rec.Write(onWire(t, ex.request, ex.segmented, nil)); err != nil {
			t.Fatalf("exchange %d: recording the request: %v", i, err)
		}
		readExactly(t, rec, len(onWire(t, ex.response, ex.segmented, nil)))
	}
	if err := rec.Close(); err != nil {
		t.Fatalf("closing the recorder: %v", err)
	}
	...
	replayed := append(append([]exchange{}, handshake...), queries(999999)...)

	rep, err := replayer.NewConnectionReplayer(fname, nil)
	if err != nil {
		t.Fatalf("NewConnectionReplayer: %v", err)
	}
	for i, ex := range replayed {
		if _, err := rep.Write(onWire(t, ex.request, ex.segmented, nil)); err != nil {
			t.Fatalf("exchange %d: replaying the request: %v", i, err)
		}

		want, err := expectedResponseWire(ex, nil)
		if err != nil {
			t.Fatalf("exchange %d: %v", i, err)
		}
