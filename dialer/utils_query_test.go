...
// hash at all.
//
// The replayer scans for the first matching hash over a slice built by iterating a
// map, so that was not a near miss: a recording holding two distinct queries served
// an arbitrary one of their responses, chosen differently from run to run, with
// nothing failing.
