...
func TestPutGetQueryValuesClearsReferences(t *testing.T) {
	s := getQueryValues(4)
	// Probe the full backing array, not just s[:len]. putQueryValues clears
	// s[:cap(s)], so a regression to clear(s) (length only) would leave the
	// tail elements holding references that a later larger get could reuse.
	full := s[:cap(s)]
	s[0].name = "col1"
	s[0].value = []byte("data")
	s[1].name = "col2"
	s[1].value = []byte("more")
	s[2].isUnset = true
	// Dirty the last slot in the backing capacity (beyond len(s)).
