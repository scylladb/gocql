...
// --- Fresh allocation tests (unmarshal never reuses a pre-existing backing
// array, since Iter.Scan gives no "valid until next Scan" guarantee) ---

func TestUnmarshalListFastPath_Float32_AlwaysFreshAlloc(t *testing.T) {
	info := listTypeInfo(TypeFloat)
	...
	if &existing[:1][0] == origBacking {
		t.Fatal("expected a fresh allocation, got the pre-existing backing array reused")
	}
	for i := range input {
...
func TestUnmarshalListFastPath_Int64_AlwaysFreshAlloc(t *testing.T) {
	info := listTypeInfo(TypeBigInt)
