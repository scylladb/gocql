...
func TestDNSEndpointResolver(t *testing.T) {
	...
	t.Run("MinResolvePeriod", func(t *testing.T) {
		var mu sync.Mutex
		var calls int
		...
		resolver := newSimplePortMuxResolver(100*time.Millisecond, 1000*time.Millisecond, lookup)
		endpoint := ResolvedClientRoute{UnresolvedClientRoute: UnresolvedClientRoute{Address: "example.com"}}
