...
func (b brokenDNSResolver) LookupIP(host string) ([]net.IP, error) {
	err := errors.New("this is error comes from mocked broken resolver")
...
func TestDNSLookupError(t *testing.T) {
	...
	for _, host := range hosts {
		expected := fmt.Sprintf("failed to resolve and translate endpoint \"%s\": lookup  on %s: this is error comes from mocked broken resolver", host, host)
