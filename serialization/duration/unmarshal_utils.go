...
const (
	maxDays      = (math.MaxInt64 - math.MaxInt64%nanoDay) / nanoDay
	minDays      = -maxDays
	maxDaysNanos = maxDays * nanoDay
	minDaysNanos = minDays * nanoDay
	zeroDuration = "0s"
)
...
func DecInt64(p []byte, v *int64) error {
	...
	default:
		if p[0] != 0 || p[1] != 0 {
			return fmt.Errorf("failed to unmarshal duration: to unmarshal into (int64) the data values of the months and days should be 0")
		}
		var ok bool
...
func DecString(p []byte, v *string) error {
	...
	default:
		if p[0] != 0 || p[1] != 0 {
			return fmt.Errorf("failed to unmarshal duration: to unmarshal into (string) the data values of the months and days should be 0")
		}
		n, ok := decNanosDur(p)
		if !ok {
			return errBrokenData
...
func DecDur(p []byte, v *time.Duration) error {
	...
	default:
		if p[0] != 0 || p[1] != 0 {
			return fmt.Errorf("failed to unmarshal duration: to unmarshal into (time.Duration) the data values of the months and days should be 0")
		}
		var ok bool
...
func decReflectInt64(p []byte, v reflect.Value) error {
	...
	default:
		if p[0] != 0 || p[1] != 0 {
			return fmt.Errorf("failed to unmarshal duration: to unmarshal into (%T) the data values of the months and days value should be 0", v.Interface())
		}
		n, ok := decNanos64(p)
		if !ok {
			return errBrokenData
