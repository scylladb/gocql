...
func TestMarshalListFastPath_Int_OutOfRange(t *testing.T) {
	if math.MaxInt == math.MaxInt32 {
		t.Skip("int is 32-bit on this platform; no out-of-range value exists")
	}
	info := listTypeInfo(TypeInt)
	v := int(math.MaxInt32)
	v++
	if _, err := marshalList(info, []int{v}); err == nil {
...
func TestUnmarshalListFastPath_Int_SliceReuse(t *testing.T) {
	info := listTypeInfo(TypeInt)
	input := []int{10, 20, 30}
	data, err := marshalList(info, input)
	if err != nil {
	...
	if &dst[0] != prev {
		t.Error("slice backing array was not reused")
	}
	for i, v := range input {
		if dst[i] != v {
