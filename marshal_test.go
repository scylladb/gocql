...
// namedStrings, namedBools, namedInt16s and namedUUIDs are declared with the
// unmarshal differential tests below.
...
func TestMarshalMapDifferential(t *testing.T) {
	...
	if strconv.IntSize == 64 {
		var overflow int64 = math.MaxInt32
		overflow++
		mSIOverflow := map[string]int{"k": int(overflow)}
...
func TestUnmarshalListFastPathMatchesReflect(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:  "int8 with null element",
			info:  mk(TypeTinyInt),
			data:  buildCQLListWithNulls([]byte{0x7F}, nil),
			dst:   func() any { return new([]int8) },
			named: func() any { return new(namedInt8s) },
		},
		{
			name:  "int8 negative and empty elements",
			info:  mk(TypeTinyInt),
			data:  buildCQLListWithNulls([]byte{0x80}, []byte{}),
			dst:   func() any { return new([]int8) },
			named: func() any { return new(namedInt8s) },
		},
		{
			name:  "int8 empty list",
			info:  mk(TypeTinyInt),
			data:  buildCQLList(),
			dst:   func() any { return new([]int8) },
			named: func() any { return new(namedInt8s) },
